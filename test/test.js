...
tape( 'main export is a function', function test( t ) {
	...
	t.strictEqual( typeof gnansum, 'function', 'main export is a function' );
	t.end();
});

tape( 'the function has an arity of 1', function test( t ) {
	t.strictEqual( gnansum.length, 1, 'has expected arity' );
	t.end();
});

tape( 'the function computes the sum of a one-dimensional ndarray, ignoring `NaN` values', function test( t ) {
	var x;
	var v;

	x = [ 1.0, -2.0, -4.0, 5.0, 0.0, NaN, 3.0, 0.0, -3.0, 3.0 ];
	v = gnansum( [ vector( x, 10, 1, 0 ) ] );
	t.strictEqual( v, 3.0, 'returns expected value' );

	x = [ 1.0, -2.0, -4.0, NaN, 5.0, 0.0, 3.0 ];
	v = gnansum( [ vector( x, 7, 1, 0 ) ] );
	t.strictEqual( v, 3.0, 'returns expected value' );

	x = [ -4.0, NaN, -4.0 ];
	v = gnansum( [ vector( x, 3, 1, 0 ) ] );
	t.strictEqual( v, -8.0, 'returns expected value' );

	x = [ NaN, 4.0 ];
	v = gnansum( [ vector( x, 2, 1, 0 ) ] );
	...
	x = [ NaN ];
	v = gnansum( [ vector( x, 1, 1, 0 ) ] );
	t.strictEqual( v, 0.0, 'returns expected value' );

	x = [ 1.0, 1.0e100, 1.0, -1.0e100 ];
	v = gnansum( [ vector( x, 4, 1, 0 ) ] );
